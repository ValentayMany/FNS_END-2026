<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3 min-w-0">
                <div class="nuol-icon-wrap shrink-0" aria-hidden="true">
                ...
                <div class="min-w-0">
                    <h2 class="nuol-page-title truncate">ກວດສອບຄຳຂໍ</h2>
                    <p class="nuol-page-sub">ເລກທີ #{{ $advanceRequest->id }}</p>
                </div>
            </div>
            <a href="{{ route('approval.index') }}" class="nuol-btn-back shrink-0">
    ...
    <style>
        ...
        .badge-deputy { background: #eff6ff; color: #1d4ed8; }
        .badge-faculty { background: #f5f3ff; color: #6d28d9; }
        .badge-approved { background: #f0fdf4; color: #15803d; }
        .badge-paid { background: #f0fdfa; color: #0f766e; }
        .badge-clearing { background: #fdf2f8; color: #a21caf; }
        .badge-cleared { background: #ecfdf5; color: #047857; }
        ...
        .nuol-btn-reject {
            ...
            box-shadow: 0 4px 14px rgba(185, 28, 28, 0.26);
            transition: transform 0.2s, box-shadow 0.2s, opacity 0.2s;
            white-space: nowrap;
        }

        .nuol-btn-reject:hover {
            transform: translateY(-1px);
        ...
        @media (max-width: 640px) {
            .nuol-kv { grid-template-columns: 1fr; }
            .nuol-card-body { padding: 20px; }
            .nuol-card-header { padding: 18px 20px; }
        }
    </style>
        ...
        <div class="nuol-container space-y-5">
            ...
            <div class="nuol-card">
                <div class="nuol-card-header">
                    ...
                    <div class="nuol-card-sub">ລາຍລະອຽດສຳລັບການກວດສອບ ແລະ ອະນຸມັດ</div>
                </div>
            ...
            @if ($advanceRequest->canBeActedBy(Auth::user()))
                <div class="nuol-card">
                    <div class="nuol-card-header">
                        ...
                        <div class="nuol-card-sub">ອະນຸມັດ ຫຼື ປະຕິເສດຄຳຂໍນີ້</div>
                    </div>
                    <div class="nuol-card-body space-y-5">
                        <form method="POST" action="{{ route('approval.approve', $advanceRequest) }}" class="nuol-actions-stack">
                            @csrf
                            <div class="nuol-form-row">
                                <input type="text" name="comment" placeholder="ໝາຍເຫດ (ຖ້າມີ)" class="nuol-form-input">
                                <button type="submit" class="nuol-btn-approve">
                        ...
                        <form method="POST" action="{{ route('approval.reject', $advanceRequest) }}" class="nuol-actions-stack">
                            @csrf
                            <div class="nuol-form-row">
                                <input type="text" name="comment" placeholder="ເຫດຜົນການປະຕິເສດ *" required
                                    class="nuol-form-input">
                                <button type="submit" class="nuol-btn-reject">
                                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                        stroke-width="2.2" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5" />
            ...
            @else
                <div class="nuol-alert warn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.008v.008H12v-.008z" />
                    </svg>
                    <div>ທ່ານບໍ່ມີສິດດຳເນີນການໃນຂັ້ນຕອນນີ້</div>
                </div>
            @endif

<x-app-layout>
                                        ...
                                        <select id="department_id" name="department_id" required class="fns-select ui-input bg-white focus:ring-rose-500 focus:border-rose-500 shadow-sm transition-all text-sm w-full cursor-pointer pr-10">
                                            @foreach ($departments as $dept)
                                                @if (!str_contains($dept->department_name, 'ປະລິນ') && !str_contains($dept->department_name, 'ປະລິມ'))
                                                    <option value="{{ $dept->id }}" {{ ($dept->department_type === 'central' || $dept->department_name === 'ພາກສ່ວນກາງ' || $dept->department_name === 'ສ່ວນກາງ') ? 'selected' : '' }}>
    ...
    <script>
        ...
        const amtDisp = document.getElementById('amount_display');
        const amtVal  = document.getElementById('amount');

        function formatCommas(v) {
            if (!v && v !== 0) return '';
        ...
        if (amtDisp && amtVal) {
            amtDisp.addEventListener('input', function() {
                const digits = this.value.replace(/\D/g, '');
                if (digits) {
                    this.value = formatCommas(digits);
                    amtVal.value = digits;
                } else {
                    this.value = '';
                    amtVal.value = '';
            ...
            if (amtVal.value) {
                amtDisp.value = formatCommas(amtVal.value);
            }
        }
        ...
        function saveAndPrintDirectly() {
            ...
            if (itemName && !isNaN(amountVal) && amountVal > 0) {
            ...
            if (draftItems.length === 0) {
                alert('ກະລຸນາປ້ອນຂໍ້ມູນລາຍຈ່າຍກ່ອນບັນທຶກ');
                return;
            }

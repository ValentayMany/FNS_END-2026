    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 space-y-5">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-medium">
                            {{ $advanceRequest->requester?->full_name ?? $advanceRequest->requester?->username }}</p>
                    </div>
                    <div>
                        ...
                        @php
                            ...
                            $color = $colors[$advanceRequest->status] ?? 'bg-gray-100 text-gray-600';
                        @endphp
            ...
            @if ($advanceRequest->status === 'draft' && $advanceRequest->requester_id === Auth::id())
                <div class="bg-white rounded-xl shadow p-6">
                    <h3 class="font-semibold text-gray-700 mb-3">ດຳເນີນການ</h3>
            ...
            @if ($advanceRequest->status === 'paid' && $advanceRequest->requester_id === Auth::id())
                <div class="bg-white rounded-xl shadow p-6">
                    <h3 class="font-semibold text-gray-700 mb-3">ສົ່ງໃບສະສາງ</h3>
                    <form method="POST" action="{{ route('clearing.submit', $advanceRequest) }}">
                        @csrf
                        <button type="submit"
                            style="background:#ea580c;color:white;padding:8px 20px;border-radius:6px;font-size:14px;border:none;cursor:pointer;font-weight:600;">
                            🧾 ສົ່ງໃບສະສາງ
                        </button>
                    </form>
                </div>
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                @forelse($advanceRequest->workflowLogs as $log)
                    <div class="flex gap-3 mb-3 last:mb-0">
                        <div
                            class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-sm shrink-0">
